...
module.exports = {
    ...
    html: {
        func: (vars, args, string, cb) => {
            if ((args[0] == '--to-var' || args[0] == '-V') && args[1]) {
                vars[args[1]] = cheerio.load(string)
                return cb()
            }
            else if ((args[0] == '--from-var' || args[0] == 'f') && args[1]) {
                if (vars[args[1]])
                    return cb(null, vars[args[1]].html())
                return cb(null, '')
            }
